...
export default function EditStudentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const loadProfileAndClasses = async () => {
      try {
        ...
        setGuardianPhone(studentData.guardian_phone);
        setAddress(studentData.address);
        setDateOfBirth(studentData.date_of_birth);
        setAdmissionDate(studentData.admission_date);
        setDepartment(studentData.department);
        setClassId(studentData.class_id || '');
        setMonthlyFee(studentData.monthly_fee);
        setIsLillah(studentData.is_lillah);
        setIsHostel(studentData.is_hostel);
        setPhotoUrl(studentData.photo_url || '');

        const classList = await db.getClasses();
        setClasses(classList);
      } catch (err) {
        console.error('Error fetching student edit data:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  // Adjust monthly fee on checking Lillah Boarding
  useEffect(() => {
    if (isLillah) {
      setMonthlyFee(0);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('শিক্ষার্থীর তথ্য আপডেট করতে সমস্যা হয়েছে।');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4 bg-white p-4 rounded-xl shadow-sm border border-slate-200">
        ...
        <div>
          <h3 className="text-lg font-bold text-slate-800">শিক্ষার্থীর তথ্য সংশোধন</h3>
          <p className="text-xs text-slate-500 mt-0.5">{name} - এর ব্যক্তিগত ও ভর্তি তথ্য এডিট করুন</p>
        </div>
      </div>
